...
- **Model Library Distributed as a Python Package**: The default damage and loss model data is now resolved from the [simcenter-dlml](https://pypi.org/project/simcenter-dlml/) package, a regular pip-installed dependency of pelicun.
    ...
    - The demand-type vocabulary is now imported from the `simcenter-dlml` package, making the model library the single source of truth for the controlled vocabularies used in the model data. Each entry carries the demand's acronym and unit type (`dlml.EDP_TYPES`). Pelicun keeps its own copy of the vocabulary in `base.EDP_TYPES` (with the derived `base.EDP_to_demand_type` view), so runtime modifications of those dictionaries remain pelicun-scoped; use the new `CustomDemandTypes` option to extend the vocabulary of an assessment.
